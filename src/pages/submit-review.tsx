...
const SubmitReview: React.FC = () => {
  ...
  const defaultReview = "Aside from this hotel's rooms being a bit dated, it has a serious bug problem...";
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setResponses([{ opening: "Error", body: "Failed to submit review", closing: "Please try again later." }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        <textarea value={review} onChange={(e) => setReview(e.target.value)} className={styles.textarea} />
        <div>
          <label htmlFor="language-select" className={styles.label}>Language:</label>
          <select id="language-select" value={language} onChange={handleLanguageChange} className={styles.select}>
            <option value="en">English</option>
            <option value="es">Español</option>
          </select>
        </div>
        <button type="submit" className={styles.button} disabled={loading}>Submit Review</button>
      </form>
      {loading ? (
